...
# Replace the existing extract_features function with this one
...
def load_model(model_path):
    """Load the trained model - handles both XGBoost JSON and joblib formats"""
    try:
        if model_path.endswith('.json'):
            model = xgb.XGBClassifier()
            model.load_model(model_path)
            print(f"Model loaded from {model_path} (JSON format)")
            ...
            try:
                ...
                model = xgb.XGBClassifier()
                model.load_model(model_path)
                print(f"Model loaded from {model_path} (XGBoost format)")
...
class DroneDetector:
    ...
    # Extract features and make prediction using the comprehensive feature extraction
     features = extract_features(rx_signal, fs=self.sample_rate, n_fft=self.fft_size)
    
     if self.model is not None:
        try:
            # Make prediction using the model
            # Note: XGBoost expects a 2D array for predictions, so reshape
            ...
            # Fallback to simple energy detection if model prediction fails
